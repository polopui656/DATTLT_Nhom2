...
def start_all_tabs_sorting():
    ...
    for algo, data_copy, chart_area in algorithms:
        if algo in ["Merge Sort", "Quick Sort", "Selection Sort", "Bubble Sort", "Insertion Sort"]:
            threading.Thread(
...
title_frame = tk.Frame(win, bg="gray", height= 50, width=100)
title_frame.pack_propagate(False)  # Không cho phép thay đổi kích thước khung theo nội dung
...
# Hàm Khung Kết Quả Result
def create_result_frame(parent):
    # Tạo khung "Result" với tiêu đề "RESULT" ở góc trái trên
    bottom_frame = tk.Frame(parent, bg="white", borderwidth=2, relief=tk.SOLID)
    bottom_frame.pack(side=tk.BOTTOM, fill=tk.X, padx=10, pady=10)

    # Khung chứa tiêu đề "RESULT" nằm ở góc trái trên
    title_frame = tk.Frame(bottom_frame, bg="white")
    title_frame.place(x=10, y=-10)  # Đặt ở góc trái trên, đè lên viền khung chính

    # Đường ngang bên trái
    left_line = tk.Canvas(title_frame, width=50, height=20, bg="white", highlightthickness=0)
    left_line.create_line(0, 5, 50, 5, fill="black", width=2)
    left_line.pack(side=tk.LEFT)

    # Tiêu đề "RESULT"
    result_label = tk.Label(title_frame, text="RESULT", font=("Helvetica", 14, "bold"), bg="white")
    result_label.pack(side=tk.LEFT, padx=5)

    # Đường ngang bên phải
    right_line = tk.Canvas(title_frame, width=50, height=20, bg="white", highlightthickness=0)
    right_line.create_line(0, 5, 50, 5, fill="black", width=2)
    right_line.pack(side=tk.LEFT)
    ...
    labels = ["Time: ", "Number: ", "Result: "]
    for text in labels:
        label = tk.Label(info_frame, text=text, font=("Helvetica", 12), bg="white", anchor="w")
        label.pack(anchor="w")
...
create_result_frame(chart_area_tab1)    # Đưa bảng Result vào
...
create_result_frame(chart_area_tab2_1)    # Đưa bảng Result vào
...
create_result_frame(chart_area_tab2_2)    # Đưa bảng Result vào
...
create_result_frame(chart_area_tab3_1)    # Đưa bảng Result vào
...
create_result_frame(chart_area_tab3_2)    # Đưa bảng Result vào
...
create_result_frame(chart_area_tab3_3)    # Đưa bảng Result vào
